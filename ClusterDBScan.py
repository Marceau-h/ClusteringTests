...
def cluster(docs: List[str]) -> dict:
    ...
    for cluster_id in np.unique(dbscan.labels_):
        centroid = np.mean(X_dist_svd[dbscan.labels_ == cluster_id], axis=0)
        for i in np.where(dbscan.labels_ == cluster_id)[0]:
            points.append(
                {
                    "text": docs[i],
    ...
    clusters = {}
    for cluster_id in np.unique(dbscan.labels_):
        cluster = {docs[i] for i in range(len(docs)) if dbscan.labels_[i] == cluster_id}
